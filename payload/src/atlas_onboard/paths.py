...
# PERF: the application sub-directories below are derived from the (already
# cached) XDG base dirs and never change during a process's lifetime, yet
# each call issues a mkdir() syscall.  During a bootstrap run these helpers
# are invoked repeatedly (get_bin_dir() alone is called from chezmoi,
# agewrap and atlasrepos), so caching the resolved Path collapses the
# redundant mkdir()/stat() syscalls to a single call per directory --
# mirroring the @lru_cache already used on the get_xdg_*_home() helpers.
@lru_cache(maxsize=1)
def get_app_data_dir() -> Path:
    """Get the application's root data directory."""
    path = get_xdg_data_home() / "atlas"
    path.mkdir(parents=True, exist_ok=True)
    return path
...
def get_app_config_dir() -> Path:
    ...
    path = get_xdg_config_home() / "atlas"
    path.mkdir(parents=True, exist_ok=True)
    return path
