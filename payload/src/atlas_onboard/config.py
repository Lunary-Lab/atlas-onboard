...
def _require_https(url: HttpUrl) -> HttpUrl:
    """Reject non-HTTPS download URLs (MITM / supply-chain hardening).

    The ``age`` and ``chezmoi`` binaries and their checksum manifests are
    fetched over the network using these URLs.  The integrity guarantee of a
    downloaded binary is only as strong as the channel used to fetch its
    checksum file: over plaintext ``http://`` an active network attacker can
    serve *both* a malicious binary and a matching forged checksum, defeating
    the mandatory checksum verification.  Requiring ``https`` keeps the fetch
    authenticated and encrypted, closing that downgrade path.
    """
    if url.scheme != "https":
        raise ValueError(
            "insecure URL scheme "
            f"'{url.scheme}://' is not allowed; downloads must use https://"
        )
    return url
...
class AgeBinaryConfig(BaseModel):
    ...
    @field_validator("checksums_url")
    @classmethod
    def _checksums_url_https(cls, v: HttpUrl) -> HttpUrl:
...
class ChezmoiAsset(BaseModel):
    ...
    @field_validator("url")
    @classmethod
    def _url_https(cls, v: HttpUrl) -> HttpUrl:
        return _require_https(v)
        ...
        with httpx.Client(
            ...
            if os_arch not in platform_urls:
                raise ConfigError(
                    ...
                    f"Please create ~/.config/atlas/bootstrap.yaml manually."
                )
