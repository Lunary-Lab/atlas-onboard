...
class SecretStore:
    ...
    def _write_to_env_file(cls, key: str, value: str) -> None:
        ...
        # Write back to file.
        # SECURITY (CWE-276): the .env file holds plaintext secrets (MASTER_KEY,
        # client secrets, ...) and is the primary secret store on Linux. Creating
        # it with Path.write_text() and only chmod()-ing to 0o600 afterwards left a
        # brief TOCTOU window in which the file existed with the default,
        # umask-derived (often world-readable) permissions. Create it atomically
        # with owner-only permissions via os.open() so secrets are never exposed.
        cls.ENV_FILE.parent.mkdir(parents=True, exist_ok=True)
        content = "\n".join(env_lines) + "\n"
        fd = os.open(cls.ENV_FILE, os.O_WRONLY | os.O_CREAT | os.O_TRUNC, 0o600)
