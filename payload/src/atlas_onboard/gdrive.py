...
def install_rclone(policy_manager) -> Path:
    ...
    install_script_path: Path | None = None
    try:
        ...
        # SECURITY (CWE-377): write the installer to a securely created temp file
        # rather than a fixed, predictable path such as ``/tmp/rclone-install.sh``.
        # A fixed name in a world-writable directory let a local attacker
        # pre-create/symlink the file or race-swap its contents between the write
        # and execution.  Because the script is run with ``sudo bash``, that was a
        # local privilege-escalation (root) vector.  ``mkstemp`` atomically creates
        # a file with an unpredictable name and owner-only (0o600) permissions,
        # which root can still read for execution.
        fd, tmp_name = tempfile.mkstemp(prefix="rclone-install-", suffix=".sh")
        install_script_path = Path(tmp_name)
        ...
        if result.returncode != 0:
            raise AtlasError(f"Failed to install rclone: {result.stderr}")

        # Find rclone after installation
        rclone_path = _find_rclone()
        if not rclone_path:
            raise AtlasError("rclone installed but not found in PATH")

        console.print(f"✅ [green]rclone installed at {rclone_path}[/green]")
        return rclone_path
    except subprocess.TimeoutExpired:
        raise AtlasError("rclone installation timed out")
    except Exception as e:
        raise AtlasError(f"Failed to install rclone: {e}")
    finally:
        # Always remove the temp installer, even on error, to avoid leaving an
        # attacker-writable script behind.
...
def setup_google_drive_sync(
    ...
    token_file = Path(
        os.path.expandvars(
            config.get("token_file", "${HOME}/.config/atlas/google-drive/token.json")
        )
    )
    ...
    # Write with owner-only permissions from creation (CWE-276): the token file
    # holds the decrypted Google Drive OAuth token and must never be exposed.
    util.write_secret_file(token_file, json.dumps(token_data, indent=2))
...
"""
    ...
    # Append to existing config or create new. The rclone config embeds the
    # OAuth client secret and refresh token, so it is written with owner-only
    # permissions from creation (CWE-276) rather than chmod-ing after the fact.
    if rclone_config_file.exists():
        existing = rclone_config_file.read_text()
        if f"[{remote_name}]" not in existing:
            util.write_secret_file(rclone_config_file, existing + "\n" + rclone_config)
        else:
            # Nothing rewritten; re-assert tight perms on the existing file.
            rclone_config_file.chmod(0o600)
    else:
        util.write_secret_file(rclone_config_file, rclone_config)
