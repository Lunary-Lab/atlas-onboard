...
def run(
    ...
    if not cfg:
        ...
        console.print(
            "You can create ~/.config/atlas/bootstrap.yaml to customize settings.\n"
        )
    ...
    console.print(
        f"🚀 [bold]Starting Atlas Bootstrap[/bold] for profile: [bold cyan]{target_profile}[/bold cyan]"
    )
    ...
    try:
        ...
        # We might still need MASTER_KEY for legacy encrypted configs in atlas-keys?
        ...
        if env_dotfiles_repo:
            ...
            if should_apply:
                dotfiles_repo = Prompt.ask(
                    "Enter dotfiles repository URL (e.g., https://github.com/Lunary-Lab/atlas-dotfiles.git)"
                )
...
def decrypt_age_key(ctx: typer.Context, master_password: str, shared_secret: str):
    ...
    from . import agent, agewrap, security, util
    ...
    try:
        ...
        if not key_path.exists():
            raise errors.AtlasError(f"Encrypted key file not found at {key_path}")
        ...
        with agent.SshAgentManager() as agent_manager:
            ...
            # Wait, atlas uses age identities for git-crypt/sops/chezmoi.
            ...
            # SECURITY (CWE-276): the decrypted age identity is the master
            # secret used by chezmoi/sops/git-crypt. Create it with owner-only
            # (0o600) permissions atomically instead of write_text()+chmod(),
            # which briefly left the key readable with default permissions.
            util.write_secret_file(target_key_file, decrypted_key)
            console.print(
...
def build_doctor_summary(failures: int, total: int) -> str:
    """Build a human-friendly summary line for the ``doctor`` command.

    Returns a Rich-markup string that gives users a single, clear verdict
    plus a next step, instead of leaving them to scan every check line.
    """
    passed = total - failures
    if failures == 0:
        return (
            f"[bold green]✅ All {total} checks passed. "
            "Your environment looks healthy.[/bold green]"
        )
    plural = "s" if failures != 1 else ""
    return (
        f"[bold red]❌ {failures} of {total} check{plural} failed[/bold red] "
        f"([green]{passed} passed[/green]). "
        "Review the ❌ items above, resolve them, then re-run "
        "[bold cyan]atlas_onboard doctor[/bold cyan]."
    )
...
def doctor(ctx: typer.Context):
    ...
    console.print("[bold]🩺 Running Atlas Doctor...[/bold]")
    ...
    console.print()
    console.print(build_doctor_summary(checks_failed, checks_total))
